    ...
    public class RJDataGridView : DataGridView
    {
            ...
            if (customizable == false) // if not customizable, apply appearance settings.
            {
                if (UIAppearance.Theme == UITheme.Dark) // If the theme is dark, set the following properties
                {
                    ...
                    if (isAlternatingRowsColor) // If color alternate rows, set a color for the color of alternate rows
                    {
                        ...
                        AlternatingRowsColor = LED_DPS.Utils.ColorEditor.Lighten(RJColors.DarkActiveBackground, 5); // Background color of alternate rows
                        SelectionBackColor = LED_DPS.Utils.ColorEditor.Lighten(UIAppearance.StyleColor, 30); // Background color of the selection.
                        SelectionTextColor = Color.White; // Selection text color.
                    }
                    else // Normal row color
                    {
                        SelectionBackColor = LED_DPS.Utils.ColorEditor.Lighten(RJColors.DarkActiveBackground, 5); // Background color of the selection.
                ...
                else // If the topic is clear, set the following properties
                {
                    ...
                    if (isAlternatingRowsColor) // If color alternate rows, set a color for the color of alternate rows
                    {
                        AlternatingRowsColor = LED_DPS.Utils.ColorEditor.Lighten(UIAppearance.StyleColor, 80); // Background color of alternate rows
                        SelectionBackColor = LED_DPS.Utils.ColorEditor.Lighten(UIAppearance.StyleColor, 40); // Background color of the selection.
                        SelectionTextColor = Color.White; // Selection text color.
                    }
                    else // Normal row color
                    {
                        SelectionBackColor = LED_DPS.Utils.ColorEditor.Lighten(UIAppearance.StyleColor, 80); // Background color of the selection.
        ...
        {// Responsible for applying the rounded corners + border smoothing
            ...
            if (stopScroll == true) // If the scroll bar is not moving apply and smooth the rounded border.
                LED_DPS.Utils.RoundedControl.RegionAndSmoothed(this, borderRadius, e.Graphics);
        }
        #endregion
