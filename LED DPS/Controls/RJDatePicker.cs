...
namespace LED_DPS.RJControls
{
    ...
    public class RJDatePicker : UserControl
    {
        ...
        /// Tutorial guide: https://www.youtube.com/watch?v=IJM9SIX0pIs&t=1s (Custom data picker - No UserControl)
        /// Tutorial guide 2: https://www.youtube.com/watch?v=CkpUQYzYCC8&t=24s (Custon text box - UserControl)
        /// </summary>
        /// 
                ...
                switch (style)
                {
                    ...
                    case ControlStyle.Glass://Glass style.
                        ...
                        if (UIAppearance.Theme == UITheme.Dark)
                            BorderColor = LED_DPS.Utils.ColorEditor.Darken(UIAppearance.TextColor, 10);
        ...
        {// When the calendar icon button is clicked, the drop-down calendar will be displayed
            ...
            if (customizable)
            {
                switch (style) // Apply a highlight to the icon button
                {
                    case ControlStyle.Solid:
                        btnIcon.BackColor = LED_DPS.Utils.ColorEditor.Darken(backgroundColor, 10);
                        btnIcon.IconColor = Color.White;
                        break;
            ...
            else
            {
                ...
                {
                    case ControlStyle.Solid:
                        btnIcon.BackColor = LED_DPS.Utils.ColorEditor.Darken(UIAppearance.StyleColor, 10);
                        btnIcon.IconColor = Color.White;
                        break;
                    case ControlStyle.Glass:
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (style == ControlStyle.Glass)
            {
                ...
                LED_DPS.Utils.RoundedControl.RegionAndBorder(this, borderRadius, graph, borderColor, borderSize);
                if (borderRadius > 1) // If the radius is greater than 1, apply rounded corners to the label.
                {
                    LED_DPS.Utils.RoundedControl.RegionOnly(dateText, borderRadius - borderSize);
                    if (isDroppedDown == true) // If the drop-down list is open, draw a fill in the button area to remove visible surface from the background, you can comment the code to understand what I mean.
                    {
                        using (SolidBrush brush = new SolidBrush(borderColor))
                        {
                            ...
                            GraphicsPath path = LED_DPS.Utils.RoundedControl.GetRoundedGPath(rect, borderRadius - borderSize);
            ...
            else
            {
                ...
                LED_DPS.Utils.RoundedControl.RegionAndSmoothed(this, borderRadius, graph);
                if (borderRadius > 1) // If the radius is greater than 1, apply rounded corners to the label.
                    LED_DPS.Utils.RoundedControl.RegionOnly(dateText, borderRadius - borderSize);
            }
        }
        ...
        private void InitializeComponent()
        {
            ...
            // RJDatePicker
            // 
            this.Name = "RJDatePicker";
            this.Load += new System.EventHandler(this.RJDatePicker_Load);
            this.ResumeLayout(false);

        }

        private void RJDatePicker_Load(object sender, EventArgs e)
        {
