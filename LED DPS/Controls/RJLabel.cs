    ...
    public class RJLabel : Label
    {
        ...
        private void ApplyAppearanceSettings()
        {
            switch (style)
            {
                ...
                case LabelStyle.Description: //If the style is description
                    if (UIAppearance.Theme == LED_DPS.Settings.UITheme.Dark) //If the theme is dark style lighten the text color
                        textColor = Utils.ColorEditor.Darken(UIAppearance.TextColor, 10);
                    else //If the theme is light style darken the text color
                        textColor = Utils.ColorEditor.Lighten(UIAppearance.TextColor, 15);
                ...
                case LabelStyle.Subtitle://If the style is subtitle set the color set in the theme and a size of 12.5F
                    if (UIAppearance.Theme == LED_DPS.Settings.UITheme.Dark) //If the theme is dark style lighten the text color
                        textColor = Utils.ColorEditor.Lighten(UIAppearance.TextColor, 45);
                    else //If the theme is light style darken the text color
                        textColor = Utils.ColorEditor.Darken(UIAppearance.TextColor, 20);
                ...
                case LabelStyle.Title2:
                    if (UIAppearance.Theme == LED_DPS.Settings.UITheme.Dark) //If the theme is dark style lighten the text color
                        textColor = Utils.ColorEditor.Lighten(UIAppearance.TextColor, 50);
                    else //If the theme is light style darken the text color
                        textColor = Utils.ColorEditor.Darken(UIAppearance.TextColor, 25);
                    this.ForeColor = textColor;
                    this.Font = new Font(this.Font.Name, 14F, this.Font.Style, this.Font.Unit);
                    break;

                case LabelStyle.BarCaption://If the style is a caption of the title bar of the main form.
                    this.Font = new Font("Montserrat", 12.5F, FontStyle.Regular, GraphicsUnit.Point, ((byte)(0)));
                    ColorTitleBar();
                    break;
        ...
        private void ColorTitleBar()
        {
            if (UIAppearance.Theme == UITheme.Light && UIAppearance.Style == UIStyle.Supernova)//If the theme is light and the style is supernova, darken the text color.
                this.ForeColor = Utils.ColorEditor.Darken(UIAppearance.TextColor, 25);
                ...
                this.ForeColor = Utils.ColorEditor.Lighten(UIAppearance.TextColor, 65);
        ...
        {//If the label is of type link, change the text color when the mouse passes over it.
            if (linkLabel == true)
                this.ForeColor = Utils.ColorEditor.Lighten(UIAppearance.StyleColor, 20);
        }
