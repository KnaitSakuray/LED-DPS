...
namespace LED_DPS.RJControls
{
    public class RJButton : IconButton
    {
        ...
        /// The properties are executed alphabetically, and the Size property needs to be executed first (Before the BorderRadius property),
        /// reason why I made this property with that name (_1_Size), to validate the radius border (See the BorderRadius property).
        /// </summary>
        [Browsable(false)]
        public Size _1_Size
        {
        ...
        public int BorderRadius
        {
            ...
            set
            {
                if (value >= 0)
                {
                    // Check and validate the radius.
                    int maxRadius = this.Height / 2; // The maximum radius must be the height of the control divided by 2 (Half).
                    if (value <= maxRadius) // If the value is less than or equal to the maximum radius, assign the specified value.
                        borderRadius = value;
                    else borderRadius = maxRadius; // Otherwise, assign the maximum radius allowed.
